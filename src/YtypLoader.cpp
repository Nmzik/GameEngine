...
YtypLoader::YtypLoader(memstream& file) {
    ...
    for (auto& Block : meta.MetaBlocks) {
        ...
        case 3649811809: {
            std::memcpy(&_CMapTypes, &file.data[Block.MetaDataBlock_struct->DataPointer],
                        sizeof(CMapTypes));
            ArchetypeDefs.reserve(_CMapTypes.archetypes.Count1);
            break;
        }
        case 2195127427: {
            for (int i = 0; i < Block.MetaDataBlock_struct->DataLength / sizeof(fwArchetypeDef);
                 i++) {
                fwArchetypeDef def;
                std::memcpy(&def,
                            &file.data[Block.MetaDataBlock_struct->DataPointer +
                                       i * sizeof(fwArchetypeDef)],
                            sizeof(fwArchetypeDef));
        ...
        case 1991296364: {
            for (int i = 0; i < Block.MetaDataBlock_struct->DataLength / sizeof(CTimeArchetypeDef);
                 i++) {
                CTimeArchetypeDef def;
                std::memcpy(&def,
                            &file.data[Block.MetaDataBlock_struct->DataPointer +
                                       i * sizeof(CTimeArchetypeDef)],
                            sizeof(CTimeArchetypeDef));
        ...
        case 273704021: {
            for (int i = 0; i < Block.MetaDataBlock_struct->DataLength / sizeof(CMloArchetypeDef);
                 i++) {
                CMloArchetypeDef def;
                std::memcpy(&def,
                            &file.data[Block.MetaDataBlock_struct->DataPointer +
                                       i * sizeof(CMloArchetypeDef)],
                            sizeof(CMloArchetypeDef));
