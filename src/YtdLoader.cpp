...
void YtdLoader::Init(memstream& file, int32_t systemSize) {
    ...
    // Textures =
    // new std::unordered_map<uint32_t, GLuint>(texDictionary->TextureNameHashesPtr.EntriesCount);
	...
	TexturesHashes = new std::vector<uint32_t>();
    TexturesHashes->resize(texDictionary->TextureNameHashesPtr.EntriesCount);
    memcpy(TexturesHashes->data(), &file.data[texDictionary->TextureNameHashesPtr.EntriesPointer],
           texDictionary->TextureNameHashesPtr.EntriesCount * sizeof(uint32_t));

    if (texDictionary->Textures.getSize() != 0) {
        // uint32_t* TexturesHashes = (uint32_t*)file.read(
        // sizeof(uint32_t) * texDictionary->TextureNameHashesPtr.EntriesCount);

        for (int i = 0; i < texDictionary->Textures.getSize(); i++) {

            std::unordered_map<uint32_t, TextureManager::Texture>::iterator it =
                TextureManager::GetTextureManager().TexturesMap.find((*TexturesHashes)[i]);
            if (it == TextureManager::GetTextureManager().TexturesMap.end()) {
                ...
                TextureManager::GetTextureManager().LoadTexture((*TexturesHashes)[i], textureID);
            }
        }
    }
}
...
void YtdLoader::Remove() {
	...
	if (TexturesHashes) {
        for (auto& hash : *TexturesHashes) {
            TextureManager::GetTextureManager().RemoveTexture(hash);
        }
        delete TexturesHashes;
        TexturesHashes = nullptr;
    }
}
